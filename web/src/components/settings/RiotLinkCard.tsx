...
const ERROR_LABEL_KEYS: Record<string, string> = {
  invalid_callback: "p_setcard.riot_err_invalid_callback",
  state_mismatch: "p_setcard.riot_err_state_mismatch",
  user_denied: "p_setcard.riot_err_user_denied",
  token_exchange: "p_setcard.riot_err_token_exchange",
  account_fetch: "p_setcard.riot_err_account_fetch",
  db_update: "p_setcard.riot_err_db_update",
  not_configured: "p_setcard.riot_err_not_configured",
  session_expired: "p_setcard.riot_err_session_expired",
};
...
export function RiotLinkCard({ available, profile, loggedIn, callbackState }: Props) {
  ...
  if (!linkedProfile || !linkedProfile.summonerName) {
    return (
      <Card>
        <h2 className="font-display font-semibold">{t("p_setcard.riot_link_title")}</h2>
        <p className="text-sm text-[var(--text-muted)]">
          {t("p_setcard.riot_pitch")}
        </p>

        {callbackState?.error && ERROR_LABEL_KEYS[callbackState.error] && (
          <p className="text-xs text-[var(--red)]" role="status" aria-live="polite">
            {t(ERROR_LABEL_KEYS[callbackState.error])}
          </p>
        )}
  ...
  // ── LINKED state ────────────────────────────────────────────────────
  const handleUnlink = () => {
    if (!confirm(t("p_setcard.riot_unlink_confirm"))) return;
    setUnlinkError(null);
  ...
  return (
    <Card>
      <div className="flex items-center justify-between">
        <h2 className="font-display font-semibold">{t("p_setcard.riot_linked_title")}</h2>
        <span
          className="text-[10px] uppercase tracking-widest text-[var(--green)]"
          role="status"
          aria-live="polite"
        >
          {t("p_setcard.riot_status_linked")}
        </span>
      </div>

      {callbackState?.ok && callbackState.warn && WARN_LABEL_KEYS[callbackState.warn] && (
        <p className="text-xs text-[var(--orange)]" role="status" aria-live="polite">
          {t(WARN_LABEL_KEYS[callbackState.warn])}
        </p>
      )}
      ...
      {linkedProfile.topChampions.length > 0 && (
        <div className="space-y-2">
          <p className="text-[10px] uppercase tracking-[0.3em] text-[var(--text-muted)]">
            {t("p_player.top_n_champions", { n: linkedProfile.topChampions.length })}
          </p>
