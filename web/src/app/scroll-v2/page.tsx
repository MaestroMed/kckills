/**
 * /scroll-v2 — TikTok-native scroll, work in progress.
 *
 * This route runs in PARALLEL with /scroll (legacy) during the rework.
 * It reuses the same server-side data layer (loadRealData, getPublishedKills,
 * getTrackedRoster, etc) — only the client rendering differs. The migration
 * strategy is documented in /PLAN_TIKTOK_NATIVE.md.
 *
 * Phases:
 *   0 — empty shell, ScrollFeedV2 = passthrough to v1                ◀ HERE
 *   1 — player pool of 5 video elements teleported via portal
 *   2 — gesture-driven snap with framer-motion springs
 *   3 — buffer manager + network-aware quality switching
 *   4 — HLS adaptive streaming via hls.js (Android) / native (Safari)
 *   5 — pull-to-refresh + end-of-feed recommendation card
 *   6 — keyboard shortcuts + desktop side-by-side layout
 *   7 — production swap (rename /scroll-v2 → /scroll)
 *
 * Once Phase 7 lands, the legacy components in /components/scroll/* (excl.
 * the v2/ subfolder) get deleted and this route becomes /scroll.
 */
...
export const metadata = {
  title: "Scroll v2 (preview) — KCKILLS",
  description: "TikTok-native preview of the kill feed. Work in progress.",
  robots: { index: false, follow: false }, // don't dilute /scroll SEO during the rework
};
...
export default async function ScrollV2Page({ searchParams }: ScrollPageProps) {
  ...
  const allClips: FeedItem[] = [...filteredMoments, ...filteredVideos];
