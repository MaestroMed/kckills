...
for ($day = 0; $day -lt 5; $day++) {
    ...
    Add-Content $log "[$([DateTime]::UtcNow.ToString('s'))Z] attente de la remise à zéro du quota : $($next.ToString('s'))Z"
    Start-Sleep -Seconds ([int][Math]::Ceiling(($next - [DateTime]::UtcNow).TotalSeconds))

    & .venv\Scripts\python.exe scripts\reanalyze_published.py --limit 700 --max-usd 3 *>> $log
    ...
    Add-Content $log "[$([DateTime]::UtcNow.ToString('s'))Z] reste à réanalyser : $left"
    if ($left -eq 0) { break }
}
