...
def _build_analysis_patch(result: dict, kill: dict) -> dict:
    ...
    patch = {
        ...
        # V42 — public-API column from migration 058 (applied 2026-05-09).
        # Frontend (FeedPlayerPool, web/src/lib/supabase/kills.ts) seeks
        # the LIVE-slot video to this offset BEFORE first paint so the
        # poster frame is the action-frame instead of a random 0-sec
        # cover. We keep ai_thumbnail_timestamp_sec around for back-
        # compat with the AI-annotations consumers — both columns carry
        # the same value, and a tiny double-write is cheaper than a
        # coordinated column rename across worker + scripts + admin.
