-- Migration 093 — VS Roulette : KC ONLY (2026-07-18)
--
-- POURQUOI : fn_pick_vs_pair (059→081) et fn_top_elo_kills_v2 (064→081) ne
-- filtraient PAS tracked_team_involvement. Les morts KC publiées pour la
-- Chambre des Souffrances (team_victim) fuyaient dans la roulette /vs —
-- Mehdi a vu Humanoid (Orianna) et Isma (Xin Zhao) en duel. Règle produit
-- absolue (CLAUDE.md) : on n'affiche que les kills OÙ KC TUE.
--
-- QUOI : redéfinition IDENTIQUE à la 081 + 2 prédicats par requête :
--   AND k.tracked_team_involvement = 'team_killer'
--   AND k.kill_visible IS NOT FALSE
--
-- À appliquer via le SQL editor Supabase (comme d'habitude). Idempotent
-- (CREATE OR REPLACE). Le client a une garde re-tirage en attendant.
-- ═══════════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    WITH candidates AS (
      ...
      AND k.tracked_team_involvement = 'team_killer'
      AND k.kill_visible IS NOT FALSE
          -- 093 : KC ONLY — la roulette tirait aussi les morts KC publiées
          -- (pool de la Chambre) : Humanoid, Isma… Un site KC n'oppose que
          -- des kills DE la KC. + kill_visible (curation admin respectée).
          AND k.tracked_team_involvement = 'team_killer'
          AND k.kill_visible IS NOT FALSE
